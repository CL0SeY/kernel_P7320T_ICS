...
static struct spi_device *mb86a35s_spi_device;
...
static int mb86a35s_spi_send(unsigned char reg, unsigned int data, unsigned int size)
{
	...
	DBGPRINT(PRINT_LHEADERFMT "** reg[%02x], data[%08x], size[%02x]\n", PRINT_LHEADER, (int)reg, (int)data, (int)size);
	...
	rtn = spi_sync(mb86a35s_spi_device, &m);
...
static int mb86a35s_spi_recv(unsigned char reg, unsigned char *data, unsigned int size)
{
	...
	t[2].rx_buf = NULL;
	t[2].rx_buf = _data;
	t[2].len    = size + 1; 	/* add receive read header size */
	spi_message_add_tail(&t[2], &m);
...
static int mb86a35s_spi_stream_recv(unsigned char reg, unsigned char *data, unsigned int size)
{
	...
	t[0].tx_buf = tx_buf;
	t[0].rx_buf = NULL;
	t[0].len    = sizeof(tx_buf) + 3;
	spi_message_add_tail(&t[0], &m);
	
	t[1].tx_buf = NULL;
	t[1].rx_buf = data;
	t[1].len    = size; 	/* add receive read header size */
	spi_message_add_tail(&t[1], &m);
		       ...
		       unsigned long arg)
{
	...
	data[1] = 0x00;
	data[2] = 0xb1;
	mb86a35_i2c_sub_send(0x28, 0x32, 0x29, &data[0], PARAM_I2C_MODE_SEND_24);
	memset(data, 0, sizeof(data));
	data[0] = 0x00;
	data[1] = 0x00;
	data[2] = 0xb1;
	mb86a35_i2c_sub_send(0x28, 0x33, 0x29, &data[0], PARAM_I2C_MODE_SEND_24);
	memset(data, 0, sizeof(data));
	data[0] = 0x00;
	data[1] = 0x00;
	data[2] = 0x00;
	mb86a35_i2c_sub_send(0x28, 0x34, 0x29, &data[0], PARAM_I2C_MODE_SEND_24);
	memset(data, 0, sizeof(data));
	data[0] = 0x00;
	data[1] = 0x00;
	data[2] = 0xb1;
...
#define	TS_CONFIG_PATTERN4	( PARAM_SBER_SCLKSEL_OFF | PARAM_SBER_SCLKSEL_ON | PARAM_SBER_SBERSEL_OFF | PARAM_SBER_SBERSEL_ON	\
				| PARAM_SBER_SPACON_OFF | PARAM_SBER_SPACON_ON | PARAM_SBER_SENON_OFF | PARAM_SBER_SENON_ON | PARAM_SBER_TSCLKCTRL_ON | PARAM_SBER_TSCLKCTRL_OFF		\
				| PARAM_SBER_SLAYER_OFF | PARAM_SBER_SLAYER_A | PARAM_SBER_SLAYER_B | PARAM_SBER_SLAYER_C		\
				| PARAM_SBER_SLAYER_AB | PARAM_SBER_SLAYER_AC | PARAM_SBER_SLAYER_BC | PARAM_SBER_SLAYER_ALL )
...
int mb86a35_IOCTL_STREAM_READ_CTRL(mb86a35_cmdcontrol_t * cmdctrl, unsigned int cmd, unsigned long arg)
{
	...
	if (STREAM_READ_CTRL_user == NULL) {
		printk("mb86a35_IOCTL_STREAM_READ_CTRL STREAM_READ_CTRL_user = NULL\n");
		rtncode = -EINVAL;
	...
	if (copy_from_user(STREAM_READ_CTRL, (void *)arg, tmpsize)) {
		DBGPRINT(PRINT_LHEADERFMT "mb86a35_IOCTL_STREAM_READ_CTRL copy_from_user failed. (len:%d)\n", PRINT_LHEADER, tmpsize);
		rtncode = -EFAULT;
		goto stream_read_ctrl_return;
	}

	if (STREAM_READ_CTRL->STREAM_CTRL & ~(PARAM_STREAMREAD_START | PARAM_STREAMREAD_STOP)) {
		printk("mb86a35_IOCTL_STREAM_READ_CTRL cmd err\n");
		rtncode = -EINVAL;
		goto stream_read_ctrl_return;
	}
	...
	if (STREAM_READ_CTRL->STREAM_CTRL & PARAM_STREAMREAD_START) {
		...
		if (STREAM_READ_CTRL->READ_SIZE > MB86A35S_READ_SIZE_MAX) {
			printk("mb86a35_IOCTL_STREAM_READ_CTRL size over\n");
			rtncode = -EINVAL;
			goto stream_read_ctrl_return;
		}
		...
		rtncode = mb86a35s_spi_sub_recv(rega, sreg, regd, &rvalue, MB86A35S_SPI_WRITE_1BYTE);
		if (rtncode != 0) {
			printk("mb86a35_IOCTL_STREAM_READ_CTRL mask err\n");
			goto stream_read_ctrl_return;
		}
		...
		rtncode = mb86a35s_spi_sub_send(rega, sreg, regd, value, MB86A35S_SPI_WRITE_1BYTE);
		if (rtncode != 0) {
			printk("mb86a35_IOCTL_STREAM_READ_CTRL Stream Read Standby err\n");
			goto stream_read_ctrl_return;
		}
	
		value = (read_size & 0x00ff);
		sreg	= MB86A35S_REG_SUBR_RAMDATAL;
		rtncode = mb86a35s_spi_sub_send(rega, sreg, regd, value, MB86A35S_SPI_WRITE_1BYTE);
		if (rtncode != 0) {
			printk("mb86a35_IOCTL_STREAM_READ_CTRL send err\n");
			goto stream_read_ctrl_return;
		}
	...
	/* Stream Read STOP	*/
		...
		rtncode = mb86a35s_spi_sub_recv(rega, sreg, regd, &rvalue, MB86A35S_SPI_WRITE_1BYTE);
		if (rtncode != 0) {
			printk("mb86a35_IOCTL_STREAM_READ_CTRL IRQ Mask read err\n");
			goto stream_read_ctrl_return;
		}
...
int mb86a35_open(struct inode *inode, struct file *filp)
{
...
ssize_t mb86a35_read(struct file *filp, char *buf, size_t count, loff_t * f_pos)
{
	...
	if (read_enable_flag == 0) {
		rtncode = -EFAULT;
		printk("read request err = %d\n", rtncode);
		goto read_return;
	}
...
/**
	...
	@param  filp		[in] pointer to structure "filp".
	@param  vma		[in] pointer to vma area.
	@retval 0	normal return.
	@retval <0	The error occurred. The detailed information is set to an errno.
*/

int mb86a35_mmap(struct file *filp, struct vm_area_struct *vma)
{
	int rtncode = 0;
	unsigned long offset	= vma->vm_pgoff << PAGE_SHIFT;
	unsigned long physical	= map_physicaladdr + offset;

	if (remap_pfn_range(vma, vma->vm_start, physical >> PAGE_SHIFT, vma->vm_end - vma->vm_start, vma->vm_page_prot)){
		rtncode = -ENOMEM;
		printk(KERN_ERR "remap_pfn_range reamp failed. ret[ %d ]\n", rtncode);
		return rtncode;
	}
...
static struct file_operations mb86a35_fops = {
	...
	mmap:	mb86a35_mmap,		/* mmap() system call entry	*/
};
#endif
/* isdbtmm_mod_e_2012.06.01 */
...
#endif
{
	...
	/* for TEST	*/
	gpio_direction_output(GPIO_SPI_TEST1, 0);
	...
	if (printk_ratelimit())
		printk(KERN_INFO "mb86a35_irq_work Called. \n");
#endif
...
#if 1
{
	...
	ret = request_irq(irq, mb86a35s_irq_handler, irqflags, devname, spi);
	if (ret < 0) {
...
/* isdbtmm_mod_e_2012.06.01 */
	...
	.remove   = __devexit_p(mb86a35s_spi_remove),
};
